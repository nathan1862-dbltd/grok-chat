...
function requestCompletion(string $endpoint, string $apiKey, string $model, array $messages): array
{
    ...
    if (!is_array($response)) {
        return [null, sprintf('Grok returned an invalid response (HTTP %d).', $statusCode)];
    }
    ...
    $apiError = $response['error']['message'] ?? $response['message'] ?? null;
    return [
        null,
        is_string($apiError) && $apiError !== ''
            ? sprintf('Grok request failed (HTTP %d): %s', $statusCode, $apiError)
            : sprintf('Grok request failed with HTTP %d.', $statusCode),
    ];
}

$apiKey = configValue($envFile, ['XAI_API_KEY', 'api-key']);
$accountId = configValue($envFile, ['CLOUDFLARE_ACCOUNT_ID', 'cf-account-id']);
$gatewayId = configValue($envFile, ['CLOUDFLARE_GATEWAY_ID', 'cf-gateway-id'], 'default');
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($imageError !== null) {
        appendAssistantMessage('**Upload error:** ' . $imageError);
        redirectToChat();
    }
    ...
    if ($content === []) {
        appendAssistantMessage('Please enter a message or attach an image.');
        redirectToChat();
    }
    ...
    appendAssistantMessage($completion ?? ('**Request error:** ' . $requestError));
    redirectToChat();
}
?>
<!DOCTYPE html>
